...
enctype="multipart/form-data">
	<h1>Alterar</h1>
	<p>Código: <?= $pet->getCod_pet() ?></p>
	<input type="hidden" name="cod_pet"
	value="<?= $pet->getCod_pet()?>">
...
<p>Nome:<input type="text" name="nome"
		...
		pattern="[a-zA-Z\sçÇãÃéÉôÔ]{2,30}"
		value="<?= $pet->getNome() ?>"></p>
<p>Pelagem:<input type="color" name="pelagem"
	value="<?= $pet->getPelagem() ?>"></p>
<p>Peso:<input type="number" name="peso"
	min="0.0" max="80.0" step="0.01"
	value="<?= $pet->getPeso()?>"></p>
<p>Nascimento:<input type="date" 
	name="data_nasci" 
	value="<?= $pet->getData_nasci() ?>"></p>
...
if (isset($_POST['botao'])) {
	...
	if($_FILES['foto']['tmp_name']==""){
		...
		if(!$pet->verificaTamanho(
			...
			A imagem deve ter no máximo 65Kb!</h2>";
		}else{
